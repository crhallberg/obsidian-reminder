...
const GOOGLE_API_CLIENT_ID = "171745408375-4d66mlm9mm70kkiegghqlec6pltnujea.apps.googleusercontent.com";
const GOOGLE_API_CLIENT_SECRET = "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";
...
export class GoogleCalendarClient extends ReminderSynchronizer {
    ...
    constructor(private refreshToken: Reference<any>, private calendarId: Reference<string | undefined>) { super(); }

    setupReady() {
        return this.refreshToken.value != null
            && this.calendarId.value != null
            && this.calendarId.value.length > 0;
    }
    ...
    private async generateAccessToken(refreshToken: string) {
        ...
        if (accessToken == null) {
            throw new Error("Access Token cannot be generated from refresh token.  Maybe the refresh token is invalid or expired.");
        }
        ...
        do {
            ...
            snapshot.push(...events
                .map(event => {
                    ...
                    if (event.id == null || event.extendedProperties == null || event.extendedProperties.shared == null) {
                        console.info("Ignoring non-managed event. : %o", event);
                        return null;
                    }
                    const reminderId: string | undefined = event.extendedProperties.shared[extendedSharedProperty.reminderIdKey];
                    if (reminderId == null) {
                        console.info("Ignoring non-managed event. : %o", event);
                        return null;
                    }
        ...
        for (const change of changes) {
            switch (change.changeType) {
                case ReminderChangeType.ADD:
                    const id = await this.add(change.id, change.reminder!, defaultTime);
    ...
    private requestBody(id: string, reminder: Reminder, defaultTime: Time) {
        const time = reminder.time.add(new Date().getTimezoneOffset(), "minutes", defaultTime)
...
<h2>Actions</h2>
<ul>
<li><a href="obsidian://show-reminder?file=${window.encodeURIComponent(reminder.file)}&title=${window.encodeURIComponent(reminder.title)}">Open the reminder</a></li>
</ul>
obsidian://show-reminder?file=${window.encodeURIComponent(reminder.file)}&title=${window.encodeURIComponent(reminder.title)}
`,
            extendedProperties: {
                shared: {
